...
local ns = vim.api.nvim_create_namespace("kenjutsu_file_tree")

---@param dir string
---@param commit_id string
---@param callback fun(err: string|nil, files: kenjutsu.FileEntry[], metadata: kenjutsu.CommitMetadata|nil)
...
---@param metadata kenjutsu.CommitMetadata|nil
...
local function format_metadata_lines(metadata)
  ...
  local lines = {} ---@type kenjutsu.RenderLine[]

  if metadata.summary ~= "" then
    local text = " " .. metadata.summary
    table.insert(lines, {
      text = text,
      highlights = { { 0, #text, "KenjutsuCommitSummary" } },
    })
  end
  ...
  if metadata.author ~= "" then
    ...
    table.insert(author_ts_hls, { col, col + #author, "KenjutsuCommitAuthor" })
    col = col + #author
  end
  ...
  if metadata.timestamp ~= "" then
    ...
    table.insert(author_ts_hls, { col, col + #ts, "KenjutsuCommitTimestamp" })
    col = col + #ts
  end
  ...
  if metadata.description ~= "" then
    ...
    for _, desc_line in ipairs(vim.split(metadata.description, "\n", { plain = true })) do
      ...
      table.insert(lines, {
        text = text,
        highlights = { { 0, #text, "KenjutsuCommitDescription" } },
      })
    end
  end
...
function FileTreeState.new(dir, log_winnr)
  ...
  vim.bo[bufnr].filetype = "kenjutsu-log-files"
